...
3. For each row where Status is blank OR "Pending" (up to 2 oldest such rows this run — leave any extra Pending for a later run, to control Higgsfield credit usage):
   ...
   g. As generation proceeds, update_row to advance Progress (and Last Updated each time): 20% after the reference image, 40/60/80% after each of the 3 scenes, 90% during captions/upscaling/concatenation.
